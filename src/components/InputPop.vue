...
<script setup>
    ...
    import { pushClickHandler, popClickHandler, resetStackHandler } from '../layouts/stack';
    ...
    const pushClick = () => {
        pushClickHandler(inputValue, stackState, selectedSize);
    };

    const popClick = () => {
        popClickHandler(stackState, popValue);
    };

    // const updateStackStateHandler = (message) => {
    //     updateStackState(stackState, textAreaValue, message);
    // };

    const resetStack = () => {
        resetStackHandler(routes);
    };
